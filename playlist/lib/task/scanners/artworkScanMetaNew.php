...
$artwork_list           = getUnscannedArtListNew();
$current_album_id       = 0;
...
foreach( $artwork_list as $key => $value )
{
  ...
  {
    echo $result[ 'comments' ][ 'picture' ][ 0 ][ 'image_mime' ];
    echo 'got here! 3';
  ...
  var_dump($temp_filename);
  echo "\r\n";
  var_dump(substr($temp_data, 0, 512));
  echo "\r\n\r\n\r\n";
  ...
  if ( is_readable( $temp_dir . '/' . $temp_filename ) )
  {
    ...
    else
    {
      ...
      echo 'art exists in cache - skipping';
      $current_album_id = $value[ 'album_id' ];
      $artwork_scanner->flag_as_skipped( $value[ 'album_id' ] );
    }
  }
  else
  {
    ...
    echo 'something went wrong - the file\'s not readable!';
    $current_album_id = $value[ 'album_id' ];
    $artwork_scanner->flag_as_skipped( $value[ 'album_id' ] );
  }
}
...
function generate_thumbnail( $path, $tmp_file, $constrain, $size, $value )
{
   ...
   */
  function getUnscannedArtListNew()
  {
